...
class DatabaseHelper {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
      ...
      Future<int> save(AttendanceModel am) async {
        try{
          ...
          var res = await dbClient.rawInsert("INSERT into $TABLE ($USER_ID, $START_TIME, $START_LAT, $START_LONG, $END_TIME, $END_LAT, $END_LONG, $DATE) VALUES (?,?,?,?,?,?,?,?)", [am.user_id,am.starttime,am.startlat,am.startlong,am.endtime,am.endlat,am.endlong,dateO]);
          if(res.toString()=="1"){
    ...
    Future<int> updateStartandEnd(AttendanceGettingModel am) async {
      ...
      var res = await dbClient.rawUpdate('UPDATE $TABLE SET  $START_TIME = ?, $END_TIME = ? WHERE $DATE= \'$dateO\' AND $USER_ID = \'$userId\'',[am.starttime,am.endtime]);
      if(res.toString()=="1"){
        _writeStart(am.starttime);
  ...
  Future<int> updateStart(StartAttendanceModel am) async {
    ...
    String userId= am.user_id.toString();
    var dbClient = await db;
    var res = await dbClient.rawUpdate('UPDATE $TABLE SET $START_TIME = ?, $START_LAT = ?, $START_LONG = ? WHERE $DATE= \'$dateO\' AND $USER_ID = \'$userId\'',[am.starttime,am.startlat,am.startlong]);
    if(res.toString()=="1"){
      _writeStart(am.starttime);
    }
    return res;
  ...
  Future<int> updateEnd(EndAttendanceModel am) async {
    ...
    String userId= am.user_id.toString();
    var dbClient = await db;
    var res = await dbClient.rawUpdate('UPDATE $TABLE SET  $END_TIME = ?, $END_LAT = ?, $END_LONG = ? WHERE $DATE= \'$dateO\' AND $USER_ID = \'$userId\'',[am.endtime,am.endlat,am.endlong]);
    if(res.toString()=="1"){
      _writeEnd(am.endtime);
    }
    return res;
  ...
  Future<int> updateWStatus(WorkStatusModel wm) async{
    ...
    var dbClient = await db;
    var res=await dbClient.rawUpdate('UPDATE $TABLE SET $WORK_STATUS = ? WHERE $DATE= \'$dateO\' AND $USER_ID = \'$userId\'',[wm.workstatus]);
    if(res.toString()=="1"){
      return res;
    }
    return res;
    ...
    Future<List<AttendanceModel>> getCurrentDateTime() async {
      String dateO = DateFormat("yyyy-MM-dd").format(now).toString();
      var dbClient = await db;
      List<Map> res = await dbClient.rawQuery('SELECT $USER_ID, $START_TIME, $END_TIME, $WORK_STATUS, $DATE FROM $TABLE WHERE $DATE= \'$dateO\'');
      ...
      Future<List<AttendanceModel>> getAllAttendance() async {
           ...
           Future<String> getStringCheckData() async {
            String dateO = DateFormat("yyyy-MM-dd").format(now).toString();
            var dbClient = await db;
            var res = await dbClient.rawQuery('SELECT * FROM $TABLE WHERE $DATE<$dateO');
...
//        var result = await dbClient.rawDelete('DELETE FROM $TABLE WHERE $DATE NOT IN (SELECT * FROM $TABLE)');
        var result = await dbClient.rawDelete('DELETE FROM $TABLE WHERE $DATE>$dateO');
        //print(result);
        return result;
      }
